...
**오케스트레이터 엔지니어링(Orchestratorering)**은 여러 AI 에이전트를 하나의 통제된 워크플로우로 엮어, 각 에이전트의 출력이 다음 에이전트의 입력이 되는 **구조화된 파이프라인**을 설계하고 운용하는 엔지니어링 분야다.
...
```
오케스트레이터 = 파이프라인 구조 + 에이전트 오케스트레이션 + 피드백 루프 + 품질 게이트
```
...
- **스킬** = 공구 하나 (드라이버, 망치)
- **오케스트레이터** = 조립 라인 (공정 순서, 품질 검사, 피드백 루프, 다음 공정 추천)

스킬이 아무리 좋아도, 어떤 순서로 조합하고, 결과를 어떻게 검증하고, 실패 시 어디로 분기하는지를 결정하는 것은 오케스트레이터의 영역이다.

---

## 3. 이 대시보드의 오케스트레이터 아키텍처
...
│  │  HarnessRecommender ──→ 다음 오케스트레이터 추천             │ │
...
- 각 오케스트레이터는 자신만의 **파이프라인 구조**를 가짐
- 완료된 오케스트레이터의 **결과(findings, verdict)**가 다음 오케스트레이터의 **컨텍스트**로 전달됨
...
| 오케스트레이터 체이닝 | **완료** | 완료 후 다음 오케스트레이터 추천 |
| 스킬 레지스트리 | **완료** | 73개 스킬 인덱스, 오케스트레이터별 매핑 |
| 내장 터미널 | **완료** | xterm.js + node-pty, claude CLI 자동 실행 |

---

## 6. 진정한 오케스트레이터 엔지니어링까지의 거리

현재 시스템은 **관찰 기반(Observational)** 오케스트레이터다. Claude가 실행하는 것을 옆에서 관찰하며 대시보드를 업데이트한다. 진정한 오케스트레이터는 **지시 기반(Directive)** — 오케스트레이터가 AI에게 지시하고 결과를 검증한다.
...
목표:   오케스트레이터가 지시한다 → Claude가 실행한다 → 오케스트레이터가 검증한다 → 다음 Phase 결정
```

**현재**: SessionWatcher가 JSONL을 읽어 Phase를 *추측*한다.
**목표**: 오케스트레이터가 Phase를 *명시적으로 제어*하고, 각 Phase에서 어떤 AI에게 무엇을 지시할지 결정한다.
...
목표:   Claude가 리뷰 → 오케스트레이터가 Codex를 자동 호출 → 결과 비교 → 종합
```
...
```
현재:   testing 오케스트레이터 → (스킬 목록만 보여줌)
목표:   testing 오케스트레이터 → tdd-mastery + testing-strategies 자동 로드 → 에이전트 프롬프트에 주입
```

**현재**: `skill-categories.json`에 오케스트레이터↔스킬 매핑이 있지만, 실제로 스킬을 에이전트 프롬프트에 주입하지 않음.
...
Level 3: Directive       오케스트레이터가 AI를 직접 호출하고 결과를 검증
Level 4: Adaptive        실행 결과에 따라 파이프라인이 동적으로 변형
Level 5: Self-Improving  오케스트레이터가 자신의 성능을 측정하고 구조를 개선
```

**현재 이 도구는 Level 2에 있다.** 파이프라인의 시각화, 사용량 추적, 자동 감지는 구현했지만, 오케스트레이터가 AI를 직접 제어하지는 않는다.
...
3. **연속성(Continuity)**: 오케스트레이터 추천으로 "다음에 뭘 해야 하지?"를 AI가 제안한다. 계획 → 구현 → 리뷰 → 테스트 → 배포의 흐름이 자연스럽게 이어진다.

4. **자원 인식(Resource Awareness)**: 두 AI 구독의 사용량을 동시에 추적한다. rate limit에 빠지기 전에 작업 분배를 조정할 수 있다.

### 아직 도달하지 못한 것

이 도구는 아직 **오케스트레이터가 AI를 직접 제어하는** 단계에 이르지 못했다. 현재는 관찰자(Observer)이지 지휘자(Conductor)가 아니다.
...
이 네 가지가 구현되면, 사용자는 터미널에서 "이 프로젝트를 리팩토링해줘"라고 한 마디만 하면 된다. 오케스트레이터가 알아서 컨텍스트를 수집하고, 계획을 세우고, Codex에게 비평을 받고, 실행하고, 검증한다. 사용자는 대시보드에서 진행 상황을 지켜보며 필요할 때만 개입하면 된다.

그것이 오케스트레이터 엔지니어링의 최종 목표다.
