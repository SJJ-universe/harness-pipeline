...
The original 10-area / 100-point rubric was tight against external-product-readiness. Phase D expanded the orchestrator's UI surface, observability, and modularity beyond what 5-point caps could express. MB6 extends two areas:
...
live behind the `ORCHESTRATOR_REMOTE_MODE` feature flag (default off).
...
- **R1-h** — `src/server/remoteRunnerSetup.js` reads `ORCHESTRATOR_REMOTE_MODE` + `ORCHESTRATOR_TOKEN`, derives `jwtKey` (HKDF info=`"runner-jwt"`) and `ledgerKey` (HKDF info=`"audit-ledger"`) from same IKM with domain separation, constructs `RunnerRegistry`. Empty env → mode=off + null registry + null keys; `preview/on` without token → degraded with `error: "token_missing"`; full env → full subsystem. server.js wires both into `EvidenceLedger` (gets signing key) and `createRunnerRoutes` (gets registry + jwtKey + ledger). 8 unit + 8 integration tests covering G1 (default closed), G3-tier1 (preview round-trip), G7-adj (claim/release).
...
**Test counts grew from 936 unit / 197 integration (pre-R1) to <!-- AUTO:test-counts -->**4068 unit / 574 integration**<!-- /AUTO --> across R1-a through R1-k.** The R1 round added approximately +170 unit + +52 integration = +222 tests, all green.
...
- **R1-e-3** — `src/runner/runnerAgent.js`: ~300-line Node entrypoint. State machine: `IDLE → HANDSHAKING → RUNNING ⇄ RECONNECTING → SHUTTING_DOWN → STOPPED`. `start()` does handshake → schedule heartbeat → connect WS. Heartbeat 401 triggers re-handshake (no spin loop). WS close 1008/1011 → fatal stop; other codes → exponential backoff with full jitter, capped at `reconnectMaxMs`. `configFromEnv` reads required env (`ORCHESTRATOR_BOOTSTRAP_TOKEN/HOST_IDENTITY/ORCHESTRATOR_URL/RUN_ID/RUN_JWT`) + optional (`ORCHESTRATOR_HEARTBEAT_INTERVAL_MS/RECONNECT_BASE_MS/RECONNECT_MAX_MS/SANDBOX_CLASS`). DI for `fetch` / `WebSocketCtor` / clock / logger. `runner/index.js` (R1-f's stub at exit 78) replaced with the real entrypoint that traps SIGTERM/SIGINT for graceful shutdown. 21 unit + 4 E2E tests.
...
- Net runtime impact: a runner host can now connect to the orchestrator over `/api/runner/events`, register agents, emit hooks, and the orchestrator's `childRegistry` + audit chain + readiness rubric all observe the activity correctly. Default off (`ORCHESTRATOR_REMOTE_MODE=off`) so single-orchestrator local-mode users see no behavioural change.
...
- Single canonical working tree: `C:\Users\SJ\orchestrator-pipeline-analysis` @ `master`.
- Test counts: <!-- AUTO:test-counts -->**4068 unit / 574 integration**<!-- /AUTO --> + legacy + smoke, all green. _(line auto-derived by `npm run scorecard:sync`; do not hand-edit between markers.)_
...
- ✅ Acceptance #2: 4 audit-chain anchors captured against real
  ...
  `ORCHESTRATOR_ALLOW_INSECURE_MANIFEST_URL=1` now also relaxes the
...
With R2 explicitly running the orchestrator end-to-end on the operator's
...
`ORCHESTRATOR_REMOTE_BRIDGE_MODE`. The Safety cap extends 17 → 18 to
...
`orchestrator-start.bat`**. That qualitative shift — from "you need git
...
- OS-aware config + data dir resolution with `ORCHESTRATOR_DATA_DIR`
  portable-mode override
- Atomic install with self-healing (partial-dir sweep + sentinel
  detection); SHA256 quarantine on mismatch
- https-only manifest URL with explicit `ORCHESTRATOR_ALLOW_INSECURE_MANIFEST_URL=1`
...
- Public-sector readiness cap (~3 points): end-to-end
  ...
  400 over the wire under `ORCHESTRATOR_DEPLOYMENT_PROFILE=public-sector`
...
**policy baseline** (`ORCHESTRATOR_DEPLOYMENT_PROFILE` resolver,
...
- **Real-binary partial verification**: a probe run executed
  against a running orchestrator server with real codex.cmd captured a
...
This is the FIRST round where the orchestrator runs as an actual deployed
...
- **R2-1 — Eval orchestrator**: `Dockerfile.orchestrator` (multi-stage,
...
- **R2.5-e — Live end-to-end proof**:
  `scripts/r2-5-bridge-probe.{sh,ps1}` brings up the orchestrator with
  `ORCHESTRATOR_REMOTE_BRIDGE_MODE=dispatch`, injects one valid
...
- **R3-a** (this update) — Two-network topology landed.
  ...
  - `orchestrator-r2-operator` (non-internal) — host port mapping path,
    only the orchestrator attaches.
  - `orchestrator-r2-runner` (internal-eligible, flipped by the strict
...
- **R3-c-2** (runtime monitor) — `src/runtime/runnerStaleMonitor.js`
  ...
  `ORCHESTRATOR_RUNNER_STALE_INTERVAL_MS` env hook for ops.
...
- **D0-a — JS foundation** (commit `c0e68cc`):
  ...
  Linux XDG; `ORCHESTRATOR_DATA_DIR`/`ORCHESTRATOR_CONFIG_DIR` env overrides for
...
- **D0-b — Windows 1st-class launcher** (commit `08f02d3`):
  `orchestrator-start.bat` (UTF-8 BOM + CRLF, dev/installer 2-mode, 10s
  health budget, `ORCHESTRATOR_NO_BROWSER=1` for CI). Companion PS1 scripts:
...
- **D0-c — Mac/Linux best-effort** (same commit `08f02d3`):
  `orchestrator-start.sh` (mode 100755, identical contract: `nohup`
...
- **D0-d — Smoke test + docs** (same commit `08f02d3`):
  ...
  → resolve-paths ORCHESTRATOR_DATA_DIR override → version-install-dir
...
- **D0-e — Launcher hardening** (commit `1655e55`, this section):
  ...
  - **D0-e-1 ORCHESTRATOR_MANIFEST_URL https:// enforcement.** The manifest
    ...
    ports/paths are handled correctly. `ORCHESTRATOR_ALLOW_INSECURE_MANIFEST_URL=1`
  ...
  - **D0-e-2 Bash manifest field extraction unified through `manifest-field`.**
    ...
    orchestrator-start.sh's MIN_NODE) now go through `launcher-cli
  ...
  - **D0-e-3 Atomic install via `.install-complete` sentinel.**
    ...
    atomic at the filesystem layer (NTFS + POSIX), so orchestrator-start
  ...
  - **D0-e-4 `/api/health` discriminator + `verify-health` command.**
    ...
    those fields. `orchestrator-start.{bat,sh}` swap raw `curl /api/health`
...
orchestrator usable by an operator with their own Claude/Codex account
...
- **D1-a — credentialStore** (commit `74d708c`):
  ...
  when `ORCHESTRATOR_ALLOW_PLAINTEXT_SECRETS=1` AND `NODE_ENV !=
...
- **D1-b — profileStore** (commit `c775708`):
  ...
  under `<ORCHESTRATOR_CONFIG_DIR>/profiles.json` with atomic
...
- **D1-c — profileSpawn** (commit `37d338a`):
  ...
  `profile.secretIds`, (4) telemetry env (`ORCHESTRATOR_PROFILE_ID` +
  `ORCHESTRATOR_WORKSPACE_PATH`). Refuses to spawn when any required
...
- **D1-gov-1 + D1-gov-2 — policy baseline** (commit `d87e622`):
  ...
  `ORCHESTRATOR_DEPLOYMENT_PROFILE` (default "standard"). Public-sector
...
- **D1-g — D1 closeout + server.js wiring** (this commit):
  ...
  End-to-end live verification: orchestrator-start.bat boots →
...
running under `ORCHESTRATOR_DEPLOYMENT_PROFILE=public-sector`). That
...
`ORCHESTRATOR_DEPLOYMENT_PROFILE=public-sector` resolver flips fail-
...
- **D2-d — `scripts/setup-wizard.{js,ps1,sh}`** (interactive wizard):
    ...
    - Track selection: `--standard` / `--public-sector` flag wins,
      else `ORCHESTRATOR_DEPLOYMENT_PROFILE=public-sector` env, else
      default standard.
...
D2 cap movement (107 → 108):
  ...
  experience is now end-to-end — double-click `orchestrator-start.bat`
...
The MG round closes MF1 §4 G10 by committing to specific tech for each of MF1 §6's four open questions. `docs/remote-sandbox-impl.md` (702 lines) chooses **Docker rootless** (with daemon fallback) for `container-strict`, leaving kata/firecracker reserved for `vm-strict` in Phase R4. Hook ingress = **WS primary + HTTPS POST one-shot fallback**. JWT = **HS256, HKDF-derived from `ORCHESTRATOR_TOKEN`** (separate label from the audit-ledger HMAC key, so neither use can forge the other). Audit ledger = **extend the existing `evidenceLedger` JSONL hash chain with HMAC-SHA256 per entry** (not a switch to SQLite — current scale ~900K rows fits append-only comfortably). Plus the runner-host control plane (env-only, heartbeat-driven discovery), the 3-layer egress policy (Docker `--internal` + nftables on bridge + dnsmasq allowlist on the controlled resolver), the 3-step bootstrap handshake (bootstrap token → runnerToken → runJWT), and the 10-row failure-mode table extending MF1 §4.2. Score 98 → 99 reflects the rollout phasing concreteness — operators can now audit each Phase R1-R4 phase against named criteria.
