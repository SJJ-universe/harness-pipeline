...
test("UI-H9-a: createAuditRoutes returns 503 stub when evidenceLedger missing", async () => {
  const orchestrator = await startApp({});
  try {
    const { status, body } = await getJSON(`${orchestrator.base}/api/audit/runs/whatever`);
...
test("UI-H9-a: createAuditRoutes also 503s when ledger missing read/verifyChain", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: partial });
  try {
    const { status } = await getJSON(`${orchestrator.base}/api/audit/runs/x`);
    assert.equal(status, 503);
  } finally {
    await orchestrator.close();
  }
...
test("UI-H9-a: 400 on invalid runId (oversized)", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const longId = "x".repeat(129); // exceeds RUN_ID_MAX_LENGTH=128
    const { status, body } = await getJSON(`${orchestrator.base}/api/audit/runs/${longId}`);
...
test("UI-H9-a: 404 when ledger has no entries for runId", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const { status, body } = await getJSON(`${orchestrator.base}/api/audit/runs/missing`);
...
test("UI-H9-a: returns entries + chain verify result", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const { status, body } = await getJSON(`${orchestrator.base}/api/audit/runs/run1`);
...
test("UI-H9-a: limits to last N entries when total > limit", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const { body } = await getJSON(`${orchestrator.base}/api/audit/runs/run1?limit=3`);
...
test("UI-H9-a: verify endpoint returns chain result + 200", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const { status, body } = await getJSON(`${orchestrator.base}/api/audit/runs/run1/verify`);
    assert.equal(status, 200);
    assert.equal(body.ok, true);
    assert.equal(body.runId, "run1");
...
test("GOV-AUDIT-0: POST export ships JSON bundle for runId with entries", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const { status, body } = await postJSON(`${orchestrator.base}/api/audit/runs/run1/export`, {});
    assert.equal(status, 200);
    assert.equal(body.ok, true);
    assert.equal(body.bundle.schema, "orchestrator-auditor-bundle/v1");
...
test("GOV-AUDIT-0: POST export 404 when no entries for runId", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const { status, body } = await postJSON(`${orchestrator.base}/api/audit/runs/empty/export`, {});
    assert.equal(status, 404);
    assert.equal(body.error, AUDIT_ERROR_CODES.not_found);
  } finally {
    await orchestrator.close();
...
test("GOV-AUDIT-0: POST export 400 on invalid runId", async () => {
  ...
  try {
    ...
    const { status, body } = await postJSON(`${orchestrator.base}/api/audit/runs/${longId}/export`, {});
    assert.equal(status, 400);
    assert.equal(body.error, AUDIT_ERROR_CODES.invalid_run_id);
  } finally {
    await orchestrator.close();
...
test("GOV-AUDIT-0: POST export bundle includes seal when sealKey configured", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger, sealKey });
  try {
    const { body } = await postJSON(`${orchestrator.base}/api/audit/runs/run1/export`, {});
...
test("GOV-AUDIT-0: POST window export 400 when both bounds missing", async () => {
  ...
  const orchestrator = await startApp({ evidenceLedger: ledger });
  try {
    const { status, body } = await postJSON(`${orchestrator.base}/api/audit/export`, {});
    assert.equal(status, 400);
